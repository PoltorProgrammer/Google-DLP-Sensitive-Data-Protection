...
*   **Split view with live preview** — click any document in the list and it instantly renders in the viewer pane on the right. Pending files open the **original** (for tagging); processed files open the **anonymized result** — switch between the two with the Original/Result buttons. Pages turn with ‹ › or the arrow keys; zoom with −/+/Fit. All rendering is local (PyMuPDF).
*   **Click-to-tag** — in the original view, simply *click* names or IDs on the page to mark them for erasure everywhere. All occurrences highlight instantly, the document's tags are always visible under the viewer, and the keyword engine auto-generates full-name combinations and OCR misspellings from the words you click. Digital PDFs work offline; scanned pages offer an explicit "Detect text" button (region-pinned Cloud OCR, results kept in memory only — never written to disk).
